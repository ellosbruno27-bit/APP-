import logging
from typing import Optional, Dict, Any, List

from sqlalchemy import select, func
from sqlalchemy.ext.asyncio import AsyncSession

from models.leads import Leads

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class LeadsService:
    """Service layer for Leads operations"""

    def __init__(self, db: AsyncSession):
        self.db = db

    async def create(self, data: Dict[str, Any]) -> Optional[Leads]:
        """Create a new leads"""
        try:
            obj = Leads(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created leads with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating leads: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Leads]:
        """Get leads by ID"""
        try:
            query = select(Leads).where(Leads.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching leads {obj_id}: {str(e)}")
            raise

    async def get_list(
        self, 
        skip: int = 0, 
        limit: int = 20, 
        query_dict: Optional[Dict[str, Any]] = None,
        sort: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Get paginated list of leadss"""
        try:
            query = select(Leads)
            count_query = select(func.count(Leads.id))
            
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Leads, field):
                        query = query.where(getattr(Leads, field) == value)
                        count_query = count_query.where(getattr(Leads, field) == value)
            
            count_result = await self.db.execute(count_query)
            total = count_result.scalar()

            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Leads, field_name):
                        query = query.order_by(getattr(Leads, field_name).desc())
                else:
                    if hasattr(Leads, sort):
                        query = query.order_by(getattr(Leads, sort))
            else:
                query = query.order_by(Leads.id.desc())

            result = await self.db.execute(query.offset(skip).limit(limit))
            items = result.scalars().all()

            return {
                "items": items,
                "total": total,
                "skip": skip,
                "limit": limit,
            }
        except Exception as e:
            logger.error(f"Error fetching leads list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Leads]:
        """Update leads"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Leads {obj_id} not found for update")
                return None
            for key, value in update_data.items():
                if hasattr(obj, key):
                    setattr(obj, key, value)

            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated leads {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating leads {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete leads"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Leads {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted leads {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting leads {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Leads]:
        """Get leads by any field"""
        try:
            if not hasattr(Leads, field_name):
                raise ValueError(f"Field {field_name} does not exist on Leads")
            result = await self.db.execute(
                select(Leads).where(getattr(Leads, field_name) == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching leads by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Leads]:
        """Get list of leadss filtered by field"""
        try:
            if not hasattr(Leads, field_name):
                raise ValueError(f"Field {field_name} does not exist on Leads")
            result = await self.db.execute(
                select(Leads)
                .where(getattr(Leads, field_name) == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Leads.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching leadss by {field_name}: {str(e)}")
            raise